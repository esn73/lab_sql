...
-- SQL 문장은 세미콜론(;)으로 끝남.
-- Ctrl + Enter: 커서가 위치한 곳의 SQL 한 문장을 실행.
-- F5: 스크립트 실행. 파일의 처음부터 끝까지 모든 문장(명령어) 실행.
...
-- Oracle 데이터베이스는 테이블 이름과 컬럼 이름을 대문자로 관리.
-- 데이터베이스 테이블에 저장되는 문자열 데이터들은 대/소문자를 구별함!
desc DEPT;

-- Oracle의 데이터 타입(data type)
-- number: 숫자 타입(정수, 실수). 
-- (예) number(2): 2자리 정수(0~99), number(7, 2): 소수점 이하 2자리까지 표현 가능한 실수.
