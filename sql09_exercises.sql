...
select e.last_name, l.city
from employees e left join departments d on e.department_id = d.department_id
                            left join locations l and d.location_id = l.location_id(+);

-- 6. 2008년에 입사한 직원들의 last_name을 검색.
select e.hire_date, m.last_name
from employees e join employees m
on e.job_id = m.job_id and e.hire_date between '2008/01/01' and '2008/12/31'
...
from employees e 
        join departments d on e.department_id = d.department_id
        join locations l on d.location_id = l.location_id
        join coutries c on l.location_id = c.country_id;
...
from employees e
    ...
    join departments d on e.department_id = d.department_id
    join locactions l on d.location_id = l.location_id
    join countries c on l.country_id = c.coutry_id;
...
from employees e
    ...
    join countries c on l.country_id = c.coutry_id;
group by c.country_name, j.job_title
having c.country_name = 'Unted States of America'
