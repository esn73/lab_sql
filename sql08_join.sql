...
-- RDBMS(Realational DataBase Management System): 관계형 DB관리 시스템
...
2. ANSI 표준 문법
SELECT 컬럼, ...
FROM 테이블1 join 종류 테이블2 ON join 조건;
*/
...
-- emp 테이블에 empno, ename, deptno 값을 1004, 오쌤, 50으로 삽입(새 데이터 추가)
...
-- right outer join에서는 오른쪽 테이블에 있는 모든 부서 번호들이 검색됨.
-- 왼쪽 테이블에만 존재하는 deptno=50에 대한 정보는 검색 결과에 포함되지 않고,
-- 오른쪽 테이블에만 존재하는 deptno=40에 대한 정보는 검색 결과에 포함됨.
...
-- Selfe Join : 같은 테이블에서 join을 하는 것.
...
-- Non-equi Join : join 조건이 부등식(x > y,  < y, ...)이 되는 경우.
-- 사번, 이름, 급여, 급여등급 검색(emp, salgrade 테이블)

--Oracle
select e.empno, e.ename, e.sal, s.grade
from emp e, salgrade s
where e.sal between s.local and s.hisal;
-- where e.sal >= s.local and e.sal <= s.hisal
...
-- 부서번호, 부서이름, 부서의 사원수, 부서의 급여 최솟값, 부서의 급여 최댓값을 검색.
-- 부서번호 오름차순
-- inner join
...
-- 부서번호, 부서이름, 사번, 이름, 매니저 사번, 매니저 이름, 급여, 급여 등급을 검색.
-- 부서 테이블의 모든 부서 번호(10, 20, 30, 40)가 검색되도록.
-- 매니저가 없는 사원(KING)도 검색되도록.
-- 출력 순서: (1) 부서번호 오름차순 (2) 사번 오름차순
select d.deptno, d.dname, e1.empno, e1.ename, e1.mgr, e2.ename, e1.sal
...
select d.depno, d.dname, e1.empno, e1.ename, e1.mgr, e2.ename, e1.sal, s.grade
from dept d left join emp e1 on d.deptno = e1.deptno  left join emp e2 
    on e1.mgr = e2.empno
    left join salgrade s on e1.sal beteen s.losal and s.hisal
order by d.deptno, e1.empno;
...
-- 급여 3000 이상인 레코드만 선택. 직원이름 오름차순 정렬.
...
select e.ename, d.loc, e.sal, s.grade
from emp e join dept d on e.deptno = d.deptno
    join salgrade s on e.sal between s.losal and s.
where e.sal >= 3000
order by e.name;
