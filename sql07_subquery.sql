-- Sub Query: Query 문의 일부로 또 다른 query문이 사용되는 것.
...
select empno, ename, sal, nvl(comm,0) as commission, (sal * 12)+nvl(comm,0) as anuual_salary
from emp
...
-- 다중행 서브쿼리: 서브쿼리 결과가 행이 2개 이상인 경우.
-- 한 개 값과 단순 비교(=, !=, >, <, ...)를 할 수 없음.
...
select ename, sal
from emp
where ename !='SCOTT' and sal = (select sal from emp where ename = 'SCOTT');
...
-- any: 여러개들 중에서 적어도 하나
-- all: 여러개 모두
select sal from emp where deptno = 10; --10번 부서 직원들의 급여
select * from emp 
