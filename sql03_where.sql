...
-- 직원 테이블에서 수당(comm)이 있는(null이 아닌) 직원들의 사번, 이름, 수당을 검색.
...
-- 직원 테이블에서 급여가 2000 이상3000이하인 직원들의 사번, 이름, 직책, 급여 검색
-- 급여의 내림차순 출력
...
-- 직원 테이블에서 10번 또는 20번 부서에서 근무하는 직원들의 이름, 부서번호, 급여를 검색
-- 출력 순서는 (1) 부서 번호 (2) 이름 오름차순으로 정렬.
...
-- 테이블에서 저장된 데이터(문자열)은 대/소문자를 구분.

-- 직원 테이블에서 직책이 CLERK 또는 MANAGER인 직원들의 직책, 이름, 급여를 출력
-- 출력 순서(1) 직책 (2) 급여 오름차순으로 정렬.
select job, ename, sal
from emp
...
select *
from emp
where hiredate >= '1987/01/01' -->= to date '1987/01/01' to date가 자동으로 날짜타입으로 변환
order by hiredate;


-- to date(날짜 문자열, 날짜 포맷 문자열): 문자열을 date 타입으로 변환.
...
-- 이름이 'D'보다 큰 이름을 갖는 직원들의 레코드를 검색. 이름의 오름차순으로 출력.
...
select ename
from emp
where ename like '%A%' --이름(ename)의 글자수는 제한이 없고, A로 시작. 뒷글자포함
order by ename;

select ename
from emp
where ename like 'A_' --이름(ename)이 두 글자로 이루어져있고, A로 시작.  A뒤 한글자만 
order by ename;

select * from emp
where job like '_LERK'; -- ALERK, BLERK, ...

-- 직원 이름에 'E'가 포함된 직원들의 레코드(모든 컬럼)를 검색. 이름 오름차순으로 출력.
select * from emp
where ename like '%E%'
order by ename;

--30번 부서에서 일하는 직책이 SALES로 시작하는 직원들의 
-- 사번, 이름, 급여, 부서번호, 직책을 검색. 순서 사번 오름차순
...
select empno, deptno, ename, sal
from emp
where (deptno = 20 or deptno =30) and sal > 2000 --**괄호로 묶어주는거 유의할것
order by deptno, empno;
...
select *
from emp
where comm is null and mgr is not null and job in('MANAGER', 'CLERK') --and job ='MANAGER' or job='CLERK' , and job in ('MANAGER', 'CLERK')
order by empno;
...
where comm is null 
    and mgr is not null 
    and (job='MANAGER' or job = 'CLERK') --and job ='MANAGER' or job='CLERK' , and job in ('MANAGER', 'CLERK')
order by empno;
