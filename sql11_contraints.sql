-- 제약 조건(constraints)
-- 테이블에 데이터들이 insert될 때 조건을 만족하는 insert될 수 있도록 만들어 주는 제약.
-- 테이블을 create(생성)할 때 
-- 테이블을 alter(변경)할 때
...
values (2, '홍길동');
-- 성공(PK 만족, not null 만족). unique와 check 제약 조건은 NN은 관심 없음.


insert into ex01(ex_id, ex_name, ex_ccol)
values (3, '홍길동' , -1);
-- ex_ccol의 값은 0 이상 이어야 한다는 check 제약 조건을 위배!
...
create table ex03(
        ...
        gender  varchar2(1),        
        -- 제약 조건 선언
        constraint pk_ex03_id_ primary key (id),
        constraint nn_ex03_name check (name is not null), --**주의 nou null이라고 할 수 없음! 반드시 check를 써야함
...
-- insert할 때 다른 테이블 PK에 없는 값은 insert되지 않도록 하기 위해서.
...
create table ex_emp(
    ...
    deptno number(2) references ex_dept(deptno) -- **외래키(foreign key) --ex__dept
);
...
insert into ex_emp values (300, '오쌤', 30); -- 오류
